...
class LeechConfig:
    vocab_size: int
    d_model: int = 192                # должно быть кратно 24
    n_layers: int = 12
    n_heads: int = 8
    block_size: int = 512
    dropout: float = 0.05
...
def generate_leech_kernel(dim=24):
    ...
    Можно заменить на реальные минимальные векторы.
    """
...
class LeechAttention(nn.Module):
    def __init__(self, cfg: LeechConfig):
        ...
        self.num_blocks = self.head_dim // 24          # кол-во 24‑мерных блоков в голове

        # генерируем ядро Лича
    ...
    def forward(self, x):
        ...
        kernel = self.W_leech[0:24, 0:24]                # [24,24] – одно и то же для всех блоков
